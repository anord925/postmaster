...
my $location = $0;
$location =~ s/UpdateNavBars.pl//;

# We'll start off by confirming that we can find our way to our
# research directory on the Site
my $site_dirname = ConfirmDirectory($location.'../../Site/');

# Open up the 'posts-by-date.txt' file and read in the 4 most
# recent (if there aren't 4, then however many there are)
my $pbd_inf = OpenInputFile($site_dirname.'posts-by-date.txt');
my @Fnames;
my @Titles;
my $num_recent = 0;
...
# If there wasn't anything in our 'posts-by-date' file, something
# could be wrong...
if ($num_recent == 0) {
    die "\n  ERROR:  No entries found in 'posts-by-date.txt'\n\n";
}
...
# Now it's time for the hard work -- traverse through our whole mirror directory
...
sub TraverseSite
{
	...
	$next_entry = $next_entry.$research_path.$Fnames[$i].'">';
	$next_entry = $next_entry.$Titles[$i].'</a></li>';
	$nb = $nb.$next_entry."\n";
    }
    $nb = $nb.'</ul>'."\n";
